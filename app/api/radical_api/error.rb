module RadicalApi
  ...
  class Error < StandardError
    ...
    # @return [RadicalApi::Error]
    def self.from_response(response)
      status = response[:status].to_i
      body  = response[:body].to_s

      if klass =  case status
                  when 400 then RadicalApi::BadRequest
                  when 401 then RadicalApi::Unauthorized
                  when 403
                    if body =~ /rate limit exceeded/i
                      RadicalApi::TooManyRequests
                    elsif body =~ /login attempts exceeded/i
                      RadicalApi::TooManyLoginAttempts
                    else
                      RadicalApi::Forbidden
                    end
                  when 404 then RadicalApi::NotFound
                  when 406 then RadicalApi::NotAcceptable
                  when 422 then RadicalApi::UnprocessableEntity
                  when 500 then RadicalApi::InternalServerError
                  when 501 then RadicalApi::NotImplemented
                  when 502 then RadicalApi::BadGateway
                  when 503 then RadicalApi::ServiceUnavailable
                  end
        klass.new(response)
      end
    end
